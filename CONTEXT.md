...
public.odontogram_records: id (uuid PK), patient_id (FK patients.id), tooth_number, tooth_face, status, notes, created_by (FK auth.users.id), created_at.
  ...
  * Restricción de coexistencia de pieza completa: índice único parcial compuesto `unique_patient_tooth_absent_extraccion_idx` (creado por la migración `20260709_odontogram_unique_absent_extraccion.sql` y aplicado en el entorno de Supabase) que restringe a un máximo de una fila por combinación de `patient_id` y `tooth_number` para los estados `ausente` y `extraccion_indicada`. Esto impide tanto la coexistencia de ambos estados distintos como la duplicación de un mismo estado para una misma pieza dental de manera deliberada.
...
Odontograma Visual: Componente visual OdontogramChart.tsx completo y aceptado, con 32 dientes FDI, panel de selección, múltiples estados simultáneos por diente, y prioridad fija extraccion_indicada sobre ausente para el indicador visual de cruz. Base de datos con constraint de consistencia funcional e índice único de exclusión mutua de pieza completa. La Server Action `createOdontogramRecord` en `src/domains/clinical/odontogram/actions.ts` captura el error 23505 de PostgreSQL (violación de índice único) y lo traduce a un mensaje amigable para el usuario final. Pendiente: integración del componente con actions.ts, sin iniciar.
